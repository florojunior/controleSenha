  ...
  <v-content>
    ...
    <v-layout
      ...
      >
        <v-row>
          <v-col cols="12" class="d-flex justify-center">
            <p class="headline font-weight-regular text--secondary">Sistema de Painel de Chamadas</p>
          </v-col>
      ...
      <v-flex
            ...
            xl4
          >
          <p class="d-lg-none headline font-weight-regular text--secondary" style="text-align: center">Sistema de Painel de Chamadas</p>
          <v-alert
            :value="showAlert"
            dense
            outlined
...
import axiosLDAP from '../plugins/axios-config';
...
export default {
  ...
  data:function(){
    return {
      urlAxiosLDAPconfig:{
        url:'http://90.0.0.109/ldap/ldap/index.php/'
      },
      urlAxiosLDAP: '',
      shoSignup: false,
      usuario: {},
  ...
  methods:{
    singin(user){
      this.isLoading = true;
      axiosLDAP.post('/login',user).then(res => {
        this.isLoading = false;
        localStorage.setItem("token_painelHemoam", res.data.token);
